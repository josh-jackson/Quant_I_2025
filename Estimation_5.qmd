...
- Loss function is a way to evaluate your parameter estimates.  It is a function that describes the difference between the model predictions and your actual data (error).

- MSE is a common loss function, which is proportional to sigma/RSE/SSE, and thus what OLS uses. 

- Other functions include Mean Absolute Error (MAE), Binary Cross-Entropy, hinge, Kullback-Leibler (KL) Divergence, etc. 

## Estimators

- Estimators are our functions used to estimate our parameters. Estimators find the parameters that make the value of their loss function as small as possible

- OLS, WLS, penalized/ridge/lasso, robust, maximum likelihood (+ variants: FIML, PML, REML, QMLE), MCMC variants (Hamiltonian; Gibbs; metropolis), variational inference, etc. 

## When OLS doesn't work

-   OLS is great conceptually. It is also great for simple problems because the math is easy. 

- However, a more general framework frequently used in psychology is Maximum Likelihood (ML) which, ahem, maximizes the *likelihood* of our data. This will be necessary for anything beyond simple regressions (OLS = ML for simple regressions). 

- ML Loss function is the "negative log-likelihood"
...
- p(data | θ ) is what we typically work with

- p(data | .5) or more extreme = .1. What do we do with this? Is this what we want to know? 
...
- We sometimes want to ask what is the most likely parameter value rather than assuming one. I.e. Estimation (note, not an estimator). 

- We still can test/falsify hypotheses, but the estimation framework is more robust (and keeps you honest)
...
- We will skip the next few slides, but revist in a later slide deck
...
- Ignore previous slides for the time being
...
- With a larger sample, your estimate of RSE becomes more reliable and stable

- With larger sample, the denominator will increase (as it is a SS) and thus the SE will decrease. 
...
- Larger N, Smaller sigma and/or greater variability in X means more precision in our estimate

## SE indexes noise in estimate
...
- We have standard errors for all statistics we estimate. Each of them provide precision on the estimate of a population parameter. How confident we are in the estimate. 
...
```{r}
#| code-fold: true
ggplot(mod.1 , aes(x=.fitted, y=y1)) +
    geom_point(size = 2) +
