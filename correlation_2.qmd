---
title: Data Description and Correlations 
format: revealjs
slide-number: true
editor: visual
execute:
...
- How do we talk about and describe data?

- Descriptive stats as the machinery of our regression models

- "Fancy models are just fancy correlations"

## Why do we describe data?

- Understand your data

- Basis of regression models
...
The dataset is available on our GitHub site, for those who want to play along at home.

---

```{r}
#| code-fold: true
world = read.csv("world_happiness_2015.csv")
head(world)
```
...
- We are interested in variation (and other measures of dispersion) because we want to understand why someone has higher or lower scores  
...
```{r}
sum((world$Happiness - mean(world$Happiness))^2)
```
...
```{r}
sum((world$Happiness - mean(world$Happiness))^2)/length(world$Happiness)
```
...
ggplot(world) + geom_density(aes(x = Happiness)) +
  ylab("Density") + xlim(1,10)
```
...
```{r}
sqrt(sum((world$Happiness - mean(world$Happiness))^2)/length(world$Happiness))
```
...
```{r}
ggplot(world) + geom_density(aes(x = Life)) +
  ylab("Density") 
```
...
- In deciding about different ways to estimate a parameter (e.g., $\mu$), it is important to consider bias and efficiency.

- **Bias**: An estimator is biased if its expected value and the true value of the parameter are different. If we sample means multiple times we will eventually converge with the population mean (e.g,. picking marbles with replacement). 

- **Efficiency**: Of two alternative estimators (how we get our estimates), the more efficient one will estimate the parameter with less error for the same sample size. (For regression with outliers we may want to use WLS, LAD, or Bayesian)



## Bias 

- Variance (and standard deviation) are *biased* estimators when applied to samples. They will *underestimate* variability in the population. 
...
- Default of most statistical packages is sample variance/SD, as one is usually working with sample data 
...
var_estimates %>%
  ...
  mutate(var = factor(var, 
                      ...
                      labels = c("Population Variance",
                                 "Sample Variance", 
                                 "Estimate of Population Variance"))) %>% #lovely labels
...
- to look at how one variable relates to another we need to think in terms of how associated they are

- If you know one, can you make a better than chance guess at the other?

- Basis of scientific inquery

-------------

```{r}
#| code-fold: true
world %>% ggplot(aes(x = Happiness, y = Life)) + geom_point(size = 3) + theme_bw()
```
...
$$\large SP = {\sum{(x-\bar{x})(y-\bar{y})}}$$

**Population:**

$$SP = {\sum{{(x-\mu_{x}})(y-\mu_{y})}}$$
...
sum((world$Happiness - mean(world$Happiness))*(world$Life - mean(world$Life)))
...
sum((world2$Happiness - mean(world2$Happiness))*(world2$Life - mean(world2$Life)))

```

```{r}
cov(world2$Happiness, world2$Life,  use = "complete.obs") *(nrow(world2) - 1)
```
...
```{r}
cov(world$Happiness, world$Life,  use = "complete.obs")
```
...
$$\large r_{xy} = {\frac{\sum({z_{x}z_{y})}}{N-1}}$$
...
```{r}
cor(world$Happiness, world$Life,  use = "complete.obs")
```
...
$$\mathbf{K_{XYZ}} = \left[\begin{array}
{rrr}
