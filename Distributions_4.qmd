...
- Almost all the time we will never and can never know what the true DPG looks like. Normal typically serves as our default (based on how we measure or DV). 
...
- What if we want 20 samples of 100 people from some distribution to mimic what it would be like if we repeated our experiment multiple times?
...
ggplot(pmf_df, aes(x = x, y = pmf)) +
  geom_bar(stat = "identity") +
  ggtitle("PMF of Bernoulli Distribution, p = .3") + xlab("Number of Successes") +
  ylab("Probability") +scale_x_continuous(breaks = seq(0, 20, by = 1)) 
```
...
-   Sometimes called Gamma Poisson.
-   Negative binomial distribution is a mixture between the poisson and another distribution, gamma. 
- Lambda (λ), is not treated as a fixed number but is itself a random variable that follows a Gamma distribution
...
ggplot(normal_df, aes(x = x, y = density)) +
  ...
  # Add titles and labels
  labs(
    title = "Visualization of pnorm(2.666)",
    subtitle = "Area under the Standard Normal Curve to the left of Z = 2.666",
    x = "Z-score",
    y = "Probability Density"
  ) +
  theme_minimal(base_size = 14)
...
What Z-score corresponds to the \_\_\_th percentile?
...
# We'll create a large, normally distributed population.
population_mean <- 0
population_sd <- 1
population_size <- 100000
population <- data.frame(value = rnorm(population_size, mean = population_mean, sd = population_sd))

# --- 2. Set Simulation Parameters ---
sample_size <- 5       # A small sample size
n_simulations <- 10000 # Number of samples to draw

# Create a vector to store the t-statistic from each sample
...
for (i in 1:n_simulations) {
  ...
  # Calculate the t-statistic for this sample
  # This measures how many standard errors the sample mean is from the population mean.
...
# Create a data frame of the results
simulation_results <- data.frame(t_stat = t_statistics)

# --- 4. Visualize the Results ---
ggplot(simulation_results, aes(x = t_stat)) +
  # Plot the density of our simulated t-statistics
  geom_density(aes(color = "Simulated t-stats"), fill = "skyblue", alpha = 0.5, size = 1) +
  ...
  scale_color_manual(
    name = "Distribution",
    values = c("Simulated means" = "black", "Theoretical t-dist (df=4)" = "red", "Normal Distribution" = "darkgreen")
  ) +
  
  labs(
    title = "The t-distribution, not the normal.",
...
- PMF, PDF, CDF
- r functions
- Binomial, Bernoulli, Poisson, negative Binomial, Normal, t, F, Chi-square.
